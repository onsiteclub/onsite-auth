...
### Report 2026-01-18 #5 - Correção Webhook (AGUARDANDO APROVAÇÃO)

**Sessão de correção de bugs no webhook handler.**

#### Problema Identificado

Logs da Vercel mostraram **2 erros** no webhook:

```
1. PGRST204: Could not find the 'billing_address_city' column of 'billing_subscriptions' in the schema cache
2. RangeError: Invalid time value at Date.toISOString
```

#### Análise

**Erro 1 - Colunas inexistentes:**
O código tentava inserir campos de endereço (`billing_address_city`, `billing_address_state`, etc.) que **não existem** na tabela `billing_subscriptions` do Supabase.

**Erro 2 - Timestamp inválido:**
Alguns eventos do Stripe enviam timestamps `null` ou `0` (ex: `customer.subscription.updated` em certas condições). O código fazia `new Date(timestamp * 1000).toISOString()` sem validar, causando `RangeError`.
...
|---------|-------|--------|
| Campos de endereço | Incluía `billing_address_*`, `customer_name`, `customer_phone` | Removidos - só salva campos que existem na tabela |
| Conversão de timestamp | `new Date(timestamp * 1000).toISOString()` | Nova função `safeTimestampToISO()` com validação |
...
```typescript
{
  ...
  stripe_price_id,
  status,
  current_period_start,  // via safeTimestampToISO()
  current_period_end,    // via safeTimestampToISO()
...
A documentação HERMES.md lista colunas de endereço na tabela `billing_subscriptions`:
- `customer_name`, `customer_phone`
- `billing_address_line1`, `billing_address_line2`
- `billing_address_city`, `billing_address_state`
- `billing_address_postal_code`, `billing_address_country`

**Pergunta:** Essas colunas existem no schema real do Supabase?

- **Se SIM**: Preciso rodar migration para adicionar as colunas, e então restaurar o código que salva esses dados.
- **Se NÃO**: A documentação HERMES.md precisa ser atualizada para refletir o schema real.

#### Status

| Item | Status |
|------|--------|
| Commit | `7155c7a` - pushed to main |
| Deploy | Vercel auto-deploy em andamento |
| Teste | Aguardando deploy para testar webhook |

#### Ação Necessária de Blue

1. Confirmar quais colunas existem na tabela `billing_subscriptions`
2. Se colunas de endereço existem → fornecer SQL ou confirmar nomes exatos
3. Se não existem → aprovar remoção e atualização da documentação
